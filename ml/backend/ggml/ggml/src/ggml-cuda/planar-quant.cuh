/*
 * PlanarQuant CUDA dequantization kernels
 * Implements GPU support for 2D Givens rotation + 3-bit quantization
 */
...
// ---- PlanarQuant dequantization ----
// Block size: 128 elements, 3-bit indices + 1-bit signs + norm
// Same block layout as turbo3_0

static __constant__ float PLANAR_CENTROIDS_3BIT[8] = {
    -0.190685f, -0.117832f, -0.065717f, -0.021460f,
     0.021460f,  0.065717f,  0.117832f,  0.190685f
};

// Givens rotation parameters (cos/sin per pair)
// Precomputed for block size 128 = 64 pairs
static __constant__ float PLANAR_COS[64] = {
...
static __constant__ float PLANAR_SIN[64] = {
...
// Planar3: 3-bit PolarQuant via 2D Givens rotation, block size 128
...
static __device__ __forceinline__ void dequantize_planar3_0(const void * vx, const int64_t ib, const int iqs, float2 & v){
    ...
    const uint8_t * signs = x[ib].signs;
    
    // Byte position and bit offset for this element pair
    const int qs_byte_idx = iqs / 4;
    const int qs_bit_off = (iqs % 4) * 2;
    const int sign_byte_idx = iqs / 8;
    const int sign_bit_off = (iqs % 8);
    
    // Extract 2-bit indices for v0, v1
    ...
    // Dequantize: centroid * sign * norm
    float q0 = PLANAR_CENTROIDS_3BIT[idx0] * s0 * norm;
    float q1 = PLANAR_CENTROIDS_3BIT[idx1] * s1 * norm;
    
    // Apply inverse Givens rotation to restore correlation
    int pair_idx = iqs / 2;
    if (pair_idx < 64) {
        float c = PLANAR_COS[pair_idx];
        float s = PLANAR_SIN[pair_idx];
        // Inverse rotation: [v0, v1]^T = R^T [q0, q1]^T
        v.x = c * q0 + s * q1;
        v.y = -s * q0 + c * q1;
    } else {
        v.x = q0;
